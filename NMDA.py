        ...
        for k in range(1,numIN+1): 
            ...
            if INref[k-1] != 0:
                ...
                dvdt = (-g_L*(INv[k-1,0]-V_rest) - INe[k-1,0]*(INv[k-1,0]-E_AMPA) 
                        - INi[k-1,0]*(INv[k-1,0]-E_GABA) + noiseIN() + IN_NMDA[k-1,0])
            ...
            Mg = 1/(8+math.exp(-8*(INv[k-1,1]-0.6)))
        ...
        for k in range(1,numPN+1):
            ...
            if PNref[k-1] != 0: 
                ...
                dvdt = (-g_L*(PNv[k-1,0]-V_rest) - PNe[k-1,0]*(PNv[k-1,0]-E_AMPA) 
                        - PNi[k-1,0]*(PNv[k-1,0]-E_GABA) + noisePN() + PN_NMDA[k-1,0])
            ...
            Mg = 1/(8+math.exp(-8*(PNv[k-1,1]-0.6)))
                ...
                if (((PNspiketimes[trial-1,k-1,m-1] + PNPNSD) <= i*TSTEP) and
                   ((PNspiketimes[trial-1,k-1,m-1] + PNPNSD) > (i-1)*TSTEP)): 
                    ...
                    for n in range(1,numPN+1): 
                        ...
                        PNA[n-1,1] += PNAwf * coupling[k-1,n-1]
                        PNB[n-1,1] += PNBwf * coupling[k-1,n-1]
                        PN_NMDA[n-1,1] += -PNfac*(PNB[n-1,1] - PNA[n-1,1])*Mg*(PNv[n-1,1]-E_NMDA)
                ...
                if (((PNspiketimes[trial-1,k-1,m-1] + PNINSD) <= i*TSTEP) and
                   ((PNspiketimes[trial-1,k-1,m-1] + PNINSD) > (i-1)*TSTEP)): 
                    ...
                    for n in range(1,numIN+1):
                        ...
                        INA[n-1,1] += INAwf * coupling[k-1,numPN+n-1]
                        INB[n-1,1] += INBwf * coupling[k-1,numPN+n-1]
                        IN_NMDA[n-1,1] += -INfac*(INB[n-1,1] - INA[n-1,1])*Mg*(INv[n-1,1]-E_NMDA)
